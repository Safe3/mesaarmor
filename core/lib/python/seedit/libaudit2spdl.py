...
def loadDict(path_inode_file):

    with open(path_inode_file, "r") as f:
        lines = f.readlines()
        i = 0
        while i+1 < len(lines):
            #print "check " + str(i+1) + ", line: " + lines[i] + ", inode: " + lines[i+1]
            if (is_number(lines[i+1])):
                ino = int(lines[i+1])
                if gInoPathDir.has_key(ino):
                    print "duplicated ino " + str(ino) + " for path " + lines[i] + ", skip"
                else:
                    gInoPathDir[ino] = lines[i]
                i = i + 2
            else:
                i = i + 1
    return True
...
def getInput(opts):
    ...
    for opt,arg in opts:
        ...
        if opt in ("-n", "--inode"):
            try:
                if loadDict(arg) == True:
                    print "loadDict done. " + str(len(gInoPathDir))
                else:
                    print "loadDict error" + arg
...
def adbShellStat(path):
    comm = "adb shell \"stat -c '%i' " + path + "\""
    print "get inode # for " + path

    result=os.popen(comm)
    inode_nums = result.readlines()
    count = len(inode_nums)
    if count == 0:
        print "no inode find for this path"
        return 0
    elif count == 1:
        return int(inode_nums[0])

    print "more than 1 inode number find, this should not happen"
    return 0

"""
use adb shell dmesg > dmesg to save avc log
use following command to get inode list
adb shell "find / -print -exec stat -c '%i' {} \; 2>/dev/null" > file_inode_list.txt
"""
def guessPathByAdbShell(line):
    """
    Guess path by find command
    """
    ino = getInode(line)
    name = getName(line)
    comm = "adb shell \"find / -name " + name + " 2>/dev/null\""
    print "##Guessing fullpath by adb shell find :%s" % (line)
...
def findPermission(doc,rule,allowtype):
    """
    ...
    for integrateTag in integrateTagList:
        if(getAttr(integrateTag,"value")==allowtype):
            allowNode=integrateTag
            break

    permTagList=allowNode.getElementsByTagName("permission")
...
def genExceptionalRule(rule, domdoc):
    """
...
def genAllowpts(rule, domdoc):
    spRuleList=[]

    generalPtsTypeList=("devpts", "ptmx")
    rolePtsTypeList=[]
    for role in gRoleList:
...
def genAllowbinder(rule, domdoc):
    """
    generate rules for binder
    """
    spRuleList=[]
    if rule["secclass"] == "binder":
        spRule = dict()
        spRule["ruletype"]="allowbinder"
        spRule["domain"]=rule["domain"]
        spRule["type"]=rule["type"]
        spRule["secclass"]=rule["secclass"]

        #TODO: how to do the check form xml directly?
        if all(x in rule["permission"] for x in ['set_context_mgr', 'transfer']):
            spRule["option"] = "m"
        elif all(x in rule["permission"] for x in ['call', 'transfer']):
            spRule["option"] = "c"
        elif 'transfer' in rule["permission"]:
            spRule["option"] = "t"
...
def genAllowprop(rule, domdoc):
    """
    generate rules for property
    """
    spRuleList=[]
    if rule["secclass"] == "property_service":
        spRule = dict()
        spRule["ruletype"]="allowprop"
        spRule["domain"]=rule["domain"]
        spRule["type"]=rule["type"]
        spRule["secclass"]=rule["secclass"]

        if 'set' in rule["permission"]:
            spRule["option"] = "set"
...
def genOther(rule, domdoc):
    ...
    spRuleList=[]
    genRuleType=["allowpriv","allowcom","allowkey"]
...
def genSPDL(rule,lines,index, domdoc):
    ...
    spRuleList = genExceptionalRule(rule, domdoc)
    if spRuleList:
        return spRuleList

    spRuleList = genAllowtty(rule, domdoc)
    if spRuleList:
        return spRuleList

    spRuleList = genAllowpts(rule, domdoc)
    if spRuleList:
        return spRuleList
    ...
    spRuleList=genAllownet(rule,line,domdoc) #use src and dest port from line
    if spRuleList:
        return spRuleList

    #allowtty,allowpts is in progress
    ################!!!!!!!!!!In progress..........
    return genOther(rule, domdoc) # line not used
...
def getFsLabelList(converterConf):
    ...
    while line:
        if(string.find(line,"supported_fs")>=0):
            ...
            list.pop(0)
            for l in list:
                label.append(l) #for Android, we don't need + "_t"
        line=fp.readline()
...
def allowbinderStr(spRule,security="low"):
    str =""
    option = spRule["option"]

    str ="allowbinder "+domainToRole(spRule["type"])+" "+option+" ;"
    return str

def allowpropStr(spRule,security="low"):
    str =""
    option = spRule["option"]

    str ="allowprop "+spRule["type"]+" "+option+" ;"
    return str
...
def allownetStr(spRule):
    ...
    if suboption=="use" and spRule["option"] == "raw":
        str ="allownet -protocol "+option+" "+suboption+";"
    elif spRule["option"] == "socket":
        if suboption in ["create","rw"]:
            if spRule["domain"] == spRule["type"]:
                str ="allownet -protocol "+option+" "+suboption+";"
            else:
                str ="allownet -protocol "+option+" -domain "+spRule["type"]+" "+suboption+";"
        else:
            print "unsupported permission for socket: " + suboption
    elif port:
        if spRule["option"] == "tcp" or spRule["option"] == "udp":
            print "src: " + src + ", dest: " + dest + ", option: " + option + ", suboption: " + suboption
            str ="allownet -protocol "+option+" -port "+port+" "+suboption+";"
    else:
        print "not generating rule for"
        print spRule
