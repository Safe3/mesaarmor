 ...
 *  		For Android, we are not using _t and _r anymore, return domain directly.
 ...
 * For Android, we skip this check
 *
 */
...
char **get_dir_list (char *fullpath,char **homedir_list, int include_myself) {
	...
	if (path[0] == '~') {
		...
		for(i=0; homedir_list[i] != NULL; i++){
            ...
            tmp_list2 = list;
			list = joint_ntarray(tmp_list2, tmp_list);
            free(tmp_list2);
		} 
		free(path);
		return list;
...
char *make_label(char *name)
{
	...
	label = (char *)my_malloc(sizeof(char)*(len+5));
//	if (head[0]=='_')
//		head++;
	sprintf(label, "f_%s", head);
	free(filename);
