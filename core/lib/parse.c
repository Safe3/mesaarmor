...
/*Permission string for binder to number*/
int str_to_binder_perm(char *s){
  if (strcmp(s, MANAGE_STR)==0){
    return MANAGE_PRM;
  }else if (strcmp(s, CALL_STR) == 0){
    return CALL_PRM;
  }else if (strcmp(s, TRANSFER_STR) == 0){
    return TRANSFER_PRM;
  }else if(strcmp(s, WILDCARD_STR) == 0){
    return MANAGE_PRM|CALL_PRM|TRANSFER_PRM;
  }
  return -1;
}

/*Permission string for svc to number*/
int str_to_svc_perm(char *s){
  if (strcmp(s, ADD_STR)==0){
    return ADD_PRM;
  }else if (strcmp(s, FIND_STR) == 0){
    return FIND_PRM;
  }else if (strcmp(s, LIST_STR) == 0){
    return LIST_PRM;
  }else if(strcmp(s, WILDCARD_STR) == 0){
    return ADD_PRM|FIND_PRM|LIST_PRM;
  }
  return -1;
}

/*Permission string for ks to number*/
int str_to_ks_perm(char *s){
  if (strcmp(s, GET_STR)==0){
    return GET_PRM;
  }else if (strcmp(s, INSERT_STR) == 0){
    return INSERT_PRM;
  }else if (strcmp(s, LOCK_STR) == 0){
    return LOCK_PRM;
  }else if (strcmp(s, SIGN_STR) == 0){
    return SIGN_PRM;
  }else if (strcmp(s, USER_STR) == 0){
    return USER_PRM;
  }else if (strcmp(s, ADDA_STR) == 0){
    return ADDA_PRM;
  }else if(strcmp(s, WILDCARD_STR) == 0){
    return GET_PRM|FIND_PRM|LOCK_PRM|SIGN_PRM|USER_PRM|ADDA_PRM;
  }
  return -1;
}
