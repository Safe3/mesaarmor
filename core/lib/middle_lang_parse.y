...
%token PROGRAM
%token <t_str> PATH
%token <t_str> PROPERTY
%token <t_str> GLOBAL_DOMAIN
%token <t_str> IDENTIFIER
%token <t_str> FILENAME
%token <t_str> NUMBER
...
%token ADMIN
%token INCLUDE
%token <t_str> INCLUDESTR
%token INLINE
%token <t_str> INLINESTR
%%
...
DOMAIN_DEF  : DOMAIN IDENTIFIER ';' {register_domain($2,0,0); }
	    | SUPERDOMAIN IDENTIFIER ';' {register_domain($2,0,1);}
	    | DOMAIN IDENTIFIER ':' SUPERDOMAINS ';' {register_domain($2,0,0);register_superdomains(superdomains_list); }
;

SUPERDOMAINS : SUPERDOMAINS ',' IDENTIFIER {superdomains_list = add_strlist(superdomains_list,$3,0);}
            |IDENTIFIER { superdomains_list = add_strlist(superdomains_list,$1,1); }
;

BODY_DEFS :ALLOW_DEF|DENY_DEF|ALLOWNET_DEF|ALLOWCOM_DEF|ALLOWPRIV_DEF|ALLOWFS_DEF|ALLOWDEV_DEF|INCLUDE_DEF|ALLOWTMP_DEF|ALLOWKEY_DEF|ALLOWBINDER_DEF|ALLOWPROP_DEF| ALLOWSVC_DEF | ALLOWKS_DEF | ALLOWDRM_DEF
          |BODY_DEFS ALLOW_DEF
          |BODY_DEFS DENY_DEF
          ...
          |BODY_DEFS ALLOWBINDER_DEF
          |BODY_DEFS ALLOWPROP_DEF
          |BODY_DEFS ALLOWSVC_DEF
          |BODY_DEFS ALLOWKS_DEF
          |BODY_DEFS ALLOWDRM_DEF
          |BODY_DEFS INLINE_DEF
;
...
/*allownet -protocol socket use*/
|ALLOWNET PROTOCOLOPT SOCKET CREATE_RW_PERM_DEF ';' {
domains_list = add_strlist(domains_list,"self",1);
register_net_sock_acl(ALLOW_RULE, NET_SOCKET, permission, NULL, domains_list);}
|ALLOWNET PROTOCOLOPT SOCKET DOMAINOPT DOMAINS_DEF CREATE_RW_PERM_DEF ';' {
register_net_sock_acl(ALLOW_RULE, NET_SOCKET, permission, NULL, domains_list);}
...
CREATE_RW_PERM_DEF:
CREATE_RW_PERM_DEF ',' CREATE {permission|=NET_CREATE;}
| CREATE_RW_PERM_DEF ',' RW {permission|=NET_RW;}
| CREATE {permission = NET_CREATE;}
| RW {permission = NET_RW;}
;
...
ALLOWBINDER_DEF:ALLOWBINDER DOMAINS_DEF BINDER_PERMISSIONS ';' {register_binder_acl(domains_list);}
;

BINDER_PERMISSIONS : BINDER_PERMISSIONS ',' IDENTIFIER {add_permission($3,BINDER_PERM,0);}
                |IDENTIFIER { add_permission($1,BINDER_PERM,1); }
                |ASTERISK { add_permission($1, BINDER_PERM, 1); }
;

ALLOWPROP_DEF:ALLOWPROP FILENAME ';' {register_prop_acl($2);}
;

ALLOWSVC_DEF : ALLOWSVC FILENAME SVC_PERMISSIONS ';' {register_svc_acl($2, 0);}
	    | ALLOWSVC IDENTIFIER SVC_PERMISSIONS ';' {register_svc_acl($2, 0);}
	    | ALLOWSVC ASTERISK SVC_PERMISSIONS ';' {register_svc_acl($2, 0);}
	    | ALLOWSVC NAME IDENTIFIER SVC_PERMISSIONS ';' {register_svc_acl($3, 1);}
;

SVC_PERMISSIONS : SVC_PERMISSIONS ',' IDENTIFIER {add_permission($3,SVC_PERM,0);}
		|IDENTIFIER {add_permission($1,SVC_PERM,1);}
		|ASTERISK {add_permission($1,SVC_PERM,1);}
;

ALLOWKS_DEF : ALLOWKS IDENTIFIER KS_PERMISSIONS ';' {register_ks_acl($2);}
;

KS_PERMISSIONS : KS_PERMISSIONS ',' IDENTIFIER {add_permission($3,KS_PERM,0);}
	       | IDENTIFIER {add_permission($1,KS_PERM,1);}
	       | ASTERISK {add_permission($1,KS_PERM,1);}
;

ALLOWDRM_DEF : ALLOWDRM IDENTIFIER ';' {register_drm_acl($2);}
;
...
INLINE_DEF: INLINE INLINESTR {inline_rule($2);}
