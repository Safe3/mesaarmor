...
void out_file_contexts_general(FILE *file_contexts, int (*not_generate_condition)(char *)) {
	int i;
	FILE_LABEL *fl;
	char fc_str[MAX_FILENAME + 1] = "";
	char fc_child_str[MAX_FILENAME*2] = "";
	char prev_fc_str[MAX_FILENAME + 1] = "";//to eleminate multiple same specification
	...
	if(gDir_search) {
		...
		for (i = 0; i<dir_label_table->element_num; i++){
			...
			if(not_generate_condition(fl->filename))
				continue;
			fprintf(file_contexts, "#%s\t-d\tu:object_r:%s:s0\n",
...
void out_file_contexts_normal_file(FILE *out){  
  ...
  fprintf(out, "/\t u:object_r:rootfs:s0\n");
...
void out_file_contexts_special_file(FILE *file_contexts){
  ...
  fprintf(file_contexts, "/dev/[^/]*tty[^/]* \tu:object_r:tty_device_t:s0\n");
  fprintf(file_contexts, "/dev/[^/]*pty[^/]* \tu:object_r:tty_device_t:s0\n");
  fprintf(file_contexts, "/dev/pts\tu:object_r:devpts_t)\n");
  fprintf(file_contexts, "/dev/pts(/.*)?	<<none>>\n");
  fprintf(file_contexts, "/dev/vcs[^/]*\tu:object_r:vcs_device_t:s0\n");
  fprintf(file_contexts, "/dev/tty\tu:object_r:devtty_t:s0\n");
  fprintf(file_contexts, "/dev/ptmx\tu:object_r:ptmx_t:s0\n");
  ...
  fprintf(file_contexts," ifdef(`out_dummy', `\n");
  fprintf(file_contexts, "HOME_ROOT/dummy -b <<none>>\n");
  fprintf(file_contexts, "HOME_DIR/dummy -b  <<none>>\n");
  fprintf(file_contexts, "/dummy/dummy-USER -b  u:object_r:ROLE_dummy_t:s0\n");
  fprintf(file_contexts,"')\n");
  fprintf(file_contexts,"')\n");
	...
	if (gDir_search) {
		...
		for (i = 0; i<dir_label_table->element_num; i++) {
			...
			if(strcmp((fl->filename)+1, "/") == 0){
				fprintf(outfp, "%s\tu:object_r:%s:s0\n",homedir,  fl->labelname);
				fprintf(outfp, "%s/[^/]+\tu:object_r:%s:s0\n",homedir,  fl->labelname);
			} else {
				  fprintf(outfp, "%s/[^/]*%s\tu:object_r:%s:s0\n",homedir, (fl->filename)+1, fl->labelname);
			}
		}
...
resolve_label_conflict(char *name)
{
	...
	tmp = strdup(name);
	return tmp; //we don't resolve conflict now
}
