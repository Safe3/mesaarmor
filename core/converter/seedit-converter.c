...
const char usage[]= "Usage : seedit-converter [-t] [-p] -i <infile> -b <base policy dir> -a <aosp sepolicy dir> -d <device sepolicy dir> -o <output dir> --profile-data <attribute profile file>\n" \
	...
	"	-a: aosp sepolicy dir. By default it is external/sepolicy\n" \
	"	-d: device sepolicy dir. e.g. device/lge/hammerhead/sepolicy\n" \
...
int main(int argc, char **argv){
  char *outdir=NULL;
  char *basedir = NULL;
  char *aosp_sepolicy_dir = NULL;
  char *device_sepolicy_dir = NULL;
  ...
  while ((ch = getopt_long(argc, argv, "\xfe:t:po:i:b:I:c:\xfd\xff\xfc:a:d:", long_options,
			   &option_index)) != EOF) {
	  switch (ch) {
	  case 't':
		  level = atoi(optarg);
	  ...
	  case 'a':
          aosp_sepolicy_dir = optarg;
		  break;

	  case 'd':
          device_sepolicy_dir = optarg;
		  break; 
  ...
  if(aosp_sepolicy_dir == NULL) {
	  aosp_sepolicy_dir = DEFAULT_AOSP_SEPOLICY_DIR;
  }
  /*
  if (device_sepolicy_dir == NULL) {
	  perror("please use -d option to provide device sepolicy dir");
	  exit(1);
  }
  */
  set_base_policy_files(basedir, aosp_sepolicy_dir, device_sepolicy_dir);
