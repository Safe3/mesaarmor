...
check_domain_name(char *name)
{
	...
	/**
         *  to align with SEAndroid, the domain name format does not have to be formed as xxxxx_t
	 */
	if (len < 2){
...
/*
 * checks whether sourcefile is <domainname>.sp, not xxxxx_t anymore
 */
 ...
 */ 
void
register_domain(char *dname, int role_flag, int attr_flag)
{
	DOMAIN *d;
	char *name = dname;
	char *tmp = NULL;
...
int get_file_state(char *path) {
	...
	len = strlen(path);
      	...
      	if (path[len-6]=='(' && path[len-5]=='/' && path[len-4]=='.' && path[len-3] =='*' && path[len-2] ==')' && path[len-1] == '?'){
		state = FILE_DIR_WILD;
...
char *get_filename(char *path, int state){
  ...
  if (state == FILE_DIR_WILD) {
    buf[len-6]='\0';
  }else if(state == FILE_ALL_CHILD){
...
int add_filerule_to_domain(char *domain_name, char *filename, int perm, int state){
  ...
  insert_element(table, "1", filename);
...
/*
label should be <domain_prefix>_<filesystem>
...
int check_fs_tmp_name(char *domain, char *fs,  char *label){
  ...
  snprintf(must_name, len+1, "%s_%s", d_prefix, fs); 
...
insert_domain_hash_by_name(char *name)
{
	...
	newdomain->binder_rule_array = NULL;
	newdomain->binder_rule_array_num = 0;
	newdomain->prop_rule_array = NULL;
	newdomain->prop_rule_array_num = 0;
	newdomain->svc_rule_array = NULL;
	newdomain->svc_rule_array_num = 0;
	newdomain->ks_rule_array = NULL;
	newdomain->ks_rule_array_num = 0;
	newdomain->drm_rule_array = NULL;
	newdomain->drm_rule_array_num = 0;
...
void register_one_domain_trans(char *from_domain, char *path){
  ...
  if (state == FILE_DIRECT_CHILD && filename != NULL){
    /* when allow <dir>* is described, add dummy permission to dummy domain. */
    label_child_dir(filename);
  }
...
void register_program(char **paths_list, int create_domain_name_flag){
  ...
  if(create_domain_name_flag==1){
    ...
    register_domain(new_domain,0,0);
    free(new_domain);
  }
...
register_pts_acl(char *domain, int flag)
{
	...
	if (flag == 1 && domain != NULL)
	{
		work.domain_name = strdup(domain);
		work.perm = CHANGE_PRM;
	}

	if (flag == 2 && domain != NULL)
	{
		work.domain_name = strdup(domain);
		work.perm = get_tmp_perm();
	}
...
register_role(char *name)
{
	...
	/*  */
	register_domain(domain_name, 1, 0);
...
void inline_rule(char *str){
  ...
  domain = search_domain_hash(current_domain);
  work.domain = domain;
  work.rule = strndup(str+1, strlen(str)-2);

  tmp = (INLINE_RULE *)extend_array(domain->inline_rule_array, &(domain->inline_rule_array_num),sizeof(INLINE_RULE));
  tmp[domain->inline_rule_array_num -1 ] = work;
  domain->inline_rule_array = tmp;
...
int register_binder_acl(char **domain_list){
  ...
  work.target= target;
  tmp = (BINDER_RULE *)extend_array(domain->binder_rule_array, &(domain->binder_rule_array_num), sizeof(BINDER_RULE));
  tmp[domain->binder_rule_array_num -1]=work;
  domain->binder_rule_array = tmp;
  return 0;
}

int register_prop_acl(char *prop){
  ...
  work.domain = domain;
  work.permission = get_tmp_perm();
  work.prop = strdup(prop);
  work.type = ALLOW_RULE;
  
  tmp = (PROP_RULE *)extend_array(domain->prop_rule_array, &(domain->prop_rule_array_num),sizeof(PROP_RULE));
  tmp[domain->prop_rule_array_num -1 ] = work;
  domain->prop_rule_array = tmp;

  return 0;
}

void register_superdomains(char **superdomains_list){

  DOMAIN *domain;
  SUPERDOMAIN_RULE *tmp;
  SUPERDOMAIN_RULE work;
  int len;
  int i;
  char **target=NULL;
  ...
  len = get_ntarray_num(superdomains_list);  
  for(i=0;i<len;i++){
    target = extend_ntarray(target, superdomains_list[i]);
  }
  work.superdomains_list = target;
  tmp = (SUPERDOMAIN_RULE *)extend_array(domain->superdomain_rule_array, &(domain->superdomain_rule_array_num), sizeof(SUPERDOMAIN_RULE));
  tmp[domain->superdomain_rule_array_num -1]=work;
  domain->superdomain_rule_array = tmp;
}

int register_svc_acl(char *svc, int is_attr){
  ...
  work.svc = strdup(svc);
  work.type = ALLOW_RULE;
  work.is_attr = is_attr;
  work.permission = get_tmp_perm();
  
  tmp = (SVC_RULE *)extend_array(domain->svc_rule_array, &(domain->svc_rule_array_num),sizeof(SVC_RULE));
  tmp[domain->svc_rule_array_num -1 ] = work;
  domain->svc_rule_array = tmp;

  return 0;
}

int register_ks_acl(char *ks){
  ...
  tmp = (KS_RULE *)extend_array(domain->ks_rule_array, &(domain->ks_rule_array_num),sizeof(KS_RULE));
  tmp[domain->ks_rule_array_num -1 ] = work;
  domain->ks_rule_array = tmp;

  return 0;
}

int register_drm_acl(char *drm){
  ...
  tmp = (DRM_RULE *)extend_array(domain->drm_rule_array, &(domain->drm_rule_array_num),sizeof(DRM_RULE));
  tmp[domain->drm_rule_array_num -1 ] = work;
  domain->drm_rule_array = tmp;
