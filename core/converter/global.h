...
/*store allownet -tcp|-udp|-raw|-socket rule*/
/* store only one rule */
...
typedef struct prop_rule_t{
  DOMAIN *domain;	/* domain                */
  int    type;          /*DENY_RULE or ALLOW_RULE*/
  int    permission;    /* only set perm now     */
  char   *prop;		/* prop name             */
} PROP_RULE;

typedef struct superdomain_rule_t{
  DOMAIN *domain;
  char **superdomains_list; /*domain list array terminated by NULL*/
} SUPERDOMAIN_RULE;

typedef struct svc_rule_t{
  DOMAIN *domain;
  int type; /*DENY_RULE or ALLOW_RULE*/
  int permission;
  char *svc; /* service name */
  int is_attr; /* 0 - service name; 1 - attribute, group of service */
} SVC_RULE;

typedef struct ks_rule_t{
  DOMAIN *domain;
  int type; /*DENY_RULE or ALLOW_RULE*/
  int permission;
  char *ks; /* Android keystore domain name */
} KS_RULE;

typedef struct drm_rule_t{
  DOMAIN *domain;
  int type; /*DENY_RULE or ALLOW_RULE*/
  int permission;
  char *drm; /* DRM server domain name */
} DRM_RULE;
...
struct domain_t {
  ...
  int		attr_flag;		/* if "name" is attribute/superdomain, then 1 else 0 */
  ...
  INLINE_RULE *inline_rule_array;
  int inline_rule_array_num;
  ...
  BINDER_RULE *binder_rule_array;
  int binder_rule_array_num;

  PROP_RULE *prop_rule_array;
  int prop_rule_array_num;

  SUPERDOMAIN_RULE *superdomain_rule_array;
  int superdomain_rule_array_num;

  SVC_RULE *svc_rule_array;
  int svc_rule_array_num;

  KS_RULE *ks_rule_array;
  int ks_rule_array_num;

  DRM_RULE *drm_rule_array;
  int drm_rule_array_num;
...
typedef struct prop_label_t
{
	char		*propname;
	char		*labelname;
} PROP_LABEL;

typedef struct svc_label_t
{
	char		*svcname;
	char		*labelname;
} SVC_LABEL;
...
extern HASH_TABLE *prop_label_table;		/* relationship between prop and label*/
